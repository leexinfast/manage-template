<template>
    <div class="app-container">
        ...
        <el-row class="le_main">
            ...
            <el-form ref="ruleForm" :rules="rule" :model="submitForm" label-width="95px" class="le_form_valid le_form_op">
                <el-form-item label="审核结果：" prop="status">
                    <el-select v-model="submitForm.status" placeholder="请选择" size="small" clearable :disabled="loan.statusShow == '已审核'?true:false">
                        <el-option v-for="item,index in statusList" :key="index" :label="item.message" :value="item.code"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审核意见：">
                    <el-input v-model="submitForm.remark" placeholder="请输入" type="textarea" :rows="5" size="small" clearable :disabled="loan.statusShow == '已审核'?true:false"></el-input>
                </el-form-item>
            </el-form>
        </el-row>
        ...
        <el-row>
            ...
            <router-link to="/loan/secondInstance-list"><el-button size="small">返回</el-button></router-link>
        </el-row>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            initDetail() {
                Http.get('secondInstance/detail', {id:this.$route.query.id}).then(res => {
                    if(res.detail.car){
                ...
                this.$refs['ruleForm'].validate((valid) => {
                    if(valid){
                        ...
                        Http.post('submit',this.submitForm).then(res => {
                            ...
                            setTimeout(() => {
                                this.$router.push('/loan/secondInstance-list');
                            },1000);
                        }).catch(err => {
                            this.sub_confirm = false;
