<template>
    <div class="app-container">
        ...
        <el-row class="le_main">
            <h3>操作</h3>
            <el-form :inline="true" ref="ruleForm" :rules="rule" :model="submitForm" label-width="120px" class="le_form_valid le_form_op">
                <el-form-item label="首次登记日期：">
                    <el-date-picker size="small" type="date" value-format="yyyy-MM-dd" placeholder="首次登记日期" v-model="submitForm.firstRegistDate" :readonly="inputShow"></el-date-picker>
                </el-form-item>
                <el-form-item label="本次登记日期：">
                    <el-date-picker size="small" type="date" value-format="yyyy-MM-dd" placeholder="本次登记日期" v-model="submitForm.registDate" :readonly="inputShow"></el-date-picker>
                </el-form-item>
                <el-form-item label="上牌地市：">
                    <el-input class="le_input" v-model="submitForm.registCity" placeholder="请输入" size="small" clearable :readonly="inputShow"></el-input>
                </el-form-item>
                <el-form-item label="车牌号码：">
                    <el-input class="le_input" v-model="submitForm.plateNo" placeholder="请输入" size="small" clearable :readonly="inputShow"></el-input>
                </el-form-item>
            </el-form>
        </el-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            initDetail() {
                Http.get('loan/cert/detail', {loanId:this.$route.query.id}).then(res => {
                    if(res.detail.car){
                        this.car = res.detail.car;
                        let car = res.detail.car;
                        let obj = {
                            firstRegistDate:car.firstRegistDate,
                            registDate:car.registDate,
                            registCity:car.registCity,
                            registProvince:car.registProvince,
                            plateNo:car.plateNo
                        }
                        this.submitForm = obj;
                    }
            ...
            handleSave() {
                ...
                Http.posts('loan/cert/save',this.submitForm).then(res => {
            ...
            handleSubmit() {
                this.submitForm.loanId = this.$route.query.id;
                Http.posts('loan/cert/submit',this.submitForm).then(res => {
            ...
            listBack(){
                this.$router.push('/cert/cert-list')
            }
        }
    }
</script>
