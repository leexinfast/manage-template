        ...
        <el-row class="le_main">
            ...
            <el-form :inline="true" ref="ruleForm" :rules="rule" :model="submitForm" label-width="120px" class="le_form_valid le_form_op">
                <el-form-item label="抵押日期：">
                    <el-date-picker size="small" type="date" value-format="yyyy-MM-dd" placeholder="首次登记日期" v-model="submitForm.mortgageDate" :readonly="inputShow"></el-date-picker>
                </el-form-item>
                <el-form-item label="牌证移交日期：">
                    <el-date-picker size="small" type="date" value-format="yyyy-MM-dd" placeholder="本次登记日期" v-model="submitForm.certTransferDate" :readonly="inputShow"></el-date-picker>
                </el-form-item>
            </el-form>
        </el-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            initDetail() {
                Http.get('loan/mortgage/detail', {loanId:this.$route.query.id}).then(res => {
                    if(res.detail.car){
                        this.car = res.detail.car;
                        let loanDetail = res.detail.loanDetail;
                        let obj = {
                            mortgageDate:loanDetail.mortgageDate,
                            certTransferDate:loanDetail.certTransferDate
                        }
                        this.submitForm = obj;
                    }
            ...
            handleSave() {
                ...
                Http.posts('loan/mortgage/save',this.submitForm).then(res => {
            ...
            handleSubmit() {
                this.submitForm.loanId = this.$route.query.id;
                Http.posts('loan/mortgage/submit',this.submitForm).then(res => {
